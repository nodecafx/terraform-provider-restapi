...
type apiObjectOpts struct {
	...
	putPath        string
	createMethod   string
	defaultHeaders map[string]string // Optional: fallback headers for all operations
	createHeaders  map[string]string // Optional: headers specific to CREATE operations
	readHeaders    map[string]string // Optional: headers specific to READ operations
	updateHeaders  map[string]string // Optional: headers specific to UPDATE operations
	deleteHeaders  map[string]string // Optional: headers specific to DELETE operations
	readMethod     string
	readData       string
	updateMethod   string
	updateData     string
...
func NewAPIObject(iClient *APIClient, opts *apiObjectOpts) (*APIObject, error) {
	...
	obj := APIObject{
		...
		createHeaders:  opts.createHeaders,
		readHeaders:    opts.readHeaders,   // NEW
		updateHeaders:  opts.updateHeaders, // NEW
		deleteHeaders:  opts.deleteHeaders, // NEW
...
func (obj *APIObject) createObject() error {
	...
	headers := obj.mergeHeaders(obj.createHeaders)
	resultString, err := obj.apiClient.sendRequest(obj.createMethod, strings.Replace(postPath, "{id}", obj.id, -1), string(b), headers)
	if err != nil {
		return err
	}
...
func (obj *APIObject) readObject() error {
	...
	headers := obj.mergeHeaders(obj.readHeaders)
	resultString, err := obj.apiClient.sendRequest(obj.readMethod, strings.Replace(getPath, "{id}", obj.id, -1), send, headers)
	if err != nil {
...
func (obj *APIObject) updateObject() error {
	...
	headers := obj.mergeHeaders(obj.updateHeaders)
	resultString, err := obj.apiClient.sendRequest(obj.updateMethod, strings.Replace(putPath, "{id}", obj.id, -1), send, headers)
	if err != nil {
		return err
	}
...
func (obj *APIObject) deleteObject() error {
	...
	headers := obj.mergeHeaders(obj.deleteHeaders)
	_, err := obj.apiClient.sendRequest(obj.destroyMethod, strings.Replace(deletePath, "{id}", obj.id, -1), send, headers)
	if err != nil {
		return err
	}
...
func (obj *APIObject) findObject(queryString string, searchKey string, searchValue string, resultsKey string, searchData string) (map[string]interface{}, error) {
	...
	*/
	...
	resultString, err := obj.apiClient.sendRequest(obj.apiClient.readMethod, searchPath, searchData, nil)
	if err != nil {
		return objFound, err
	}
...
// mergeHeaders combines operation-specific headers with default headers and client headers
// Priority: operation-specific > default > client-level
func (obj *APIObject) mergeHeaders(operationHeaders map[string]string) map[string]string {
	result := make(map[string]string)

	// Start with client-level headers (lowest priority)
	if obj.apiClient.createHeaders != nil {
		for k, v := range obj.apiClient.createHeaders {
			result[k] = v
		}
	}

	// Add default headers (medium priority)
	if obj.defaultHeaders != nil {
		for k, v := range obj.defaultHeaders {
			result[k] = v
		}
	}

	// Add operation-specific headers (highest priority)
	if operationHeaders != nil {
