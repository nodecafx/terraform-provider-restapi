	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"extra_headers": {
				...
				MaxItems:    1,
				Optional:    true,
				Description: "Configure operation-specific headers for API requests",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"default": {
							...
							Optional:    true,
							Description: "Default headers applied to all operations unless overridden",
						},
						"create": {
							Type:        schema.TypeMap,
							Elem:        &schema.Schema{Type: schema.TypeString},
							Optional:    true,
							Description: "Headers specific to CREATE (POST) operations",
						},
						"read": {
							Type:        schema.TypeMap,
							Elem:        &schema.Schema{Type: schema.TypeString},
							Optional:    true,
							Description: "Headers specific to READ (GET) operations",
						},
						"update": {
							Type:        schema.TypeMap,
							Elem:        &schema.Schema{Type: schema.TypeString},
							Optional:    true,
							Description: "Headers specific to UPDATE (PUT) operations",
						},
						"delete": {
							Type:        schema.TypeMap,
							Elem:        &schema.Schema{Type: schema.TypeString},
							Optional:    true,
							Description: "Headers specific to DELETE operations",
						},
					},
				},
			},
...
func buildAPIObjectOpts(d *schema.ResourceData) (*apiObjectOpts, error) {
	...
	// Process extra_headers configuration
	if v, ok := d.GetOk("extra_headers"); ok {
		extraHeaders := v.([]interface{})
		if len(extraHeaders) > 0 {
			headerConfig := extraHeaders[0].(map[string]interface{})

			// Process default headers
			if defaultHeaders, exists := headerConfig["default"]; exists && defaultHeaders != nil {
				opts.defaultHeaders = make(map[string]string)
				for k, val := range defaultHeaders.(map[string]interface{}) {
					opts.defaultHeaders[k] = val.(string)
				}
			}

			// Process create headers
			if createHeaders, exists := headerConfig["create"]; exists && createHeaders != nil {
				opts.createHeaders = make(map[string]string)
				for k, val := range createHeaders.(map[string]interface{}) {
					opts.createHeaders[k] = val.(string)
				}
			}

			// Process read headers
			if readHeaders, exists := headerConfig["read"]; exists && readHeaders != nil {
				opts.readHeaders = make(map[string]string)
				for k, val := range readHeaders.(map[string]interface{}) {
					opts.readHeaders[k] = val.(string)
				}
			}

			// Process update headers
			if updateHeaders, exists := headerConfig["update"]; exists && updateHeaders != nil {
				opts.updateHeaders = make(map[string]string)
				for k, val := range updateHeaders.(map[string]interface{}) {
					opts.updateHeaders[k] = val.(string)
				}
			}

			// Process delete headers
			if deleteHeaders, exists := headerConfig["delete"]; exists && deleteHeaders != nil {
				opts.deleteHeaders = make(map[string]string)
				for k, val := range deleteHeaders.(map[string]interface{}) {
					opts.deleteHeaders[k] = val.(string)
				}
			}
		}
	}
